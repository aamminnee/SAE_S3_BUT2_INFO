    ...
    for (int index = 0; index < total_cases; index++) {
        ...
        // Case 2x1
            ...
            if (brique->stock <= 0) {
                ...
                Brique *brique1 = &briques[trouver_brique_proche(pixel, briques, nb_briques)];
                Brique *brique2 = &briques[trouver_brique_proche(pixel2, briques, nb_briques)];

                char hex1[7], hex2[7];
                pixel_en_hexadecimal(brique1->couleur, hex1);
                pixel_en_hexadecimal(brique2->couleur, hex2);
                ...
                R.prix_total += brique1->prix + brique2->prix;
                R.somme_erreurs += difference_pixel(pixel, brique1->couleur) + difference_pixel(pixel2, brique2->couleur);
                R.nb_poses += 2;

                R.lignes[index] = malloc(128);
                sprintf(R.lignes[index], "1x1/%s %d %d %d", hex1, index % img->largeur, index / img->largeur, 0);
                R.lignes[v] = malloc(128);
                sprintf(R.lignes[v], "1x1/%s %d %d %d", hex2, v % img->largeur, v / img->largeur, 0);
            } else {
