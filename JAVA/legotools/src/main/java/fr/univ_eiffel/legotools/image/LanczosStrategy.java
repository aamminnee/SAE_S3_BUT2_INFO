...
public class LanczosStrategy implements ResolutionStrategy {
	...
	@Override
    public BufferedImage resize(BufferedImage source, int targetWidth, int targetHeight) { // Correction : rescale -> resize
        ...
        for (int x = 0; x < targetWidth; x++) {
            for (int y = 0; y < targetHeight; y++) {
                ...
                // Parcours du voisinage vertical
                for (int m = -A + 1; m <= A; m++) {
                ...
                // Utilisation de toRGB (de l'interface) pour gérer le clamping et l'alpha (0xFF)
	...
	private double lanczosWeight(double x) {
		...
		if (x < 1e-5) { // Tolérance pour float
			return 1.0;
		}
	...
	private double sinc(double x) {
		...
		// Fonction sinc normalisée
		return Math.sin(Math.PI * x) / (Math.PI * x);
	}
	
	private int clamp(int v, int min, int max) {
		// Limite la valeur entre min et max (pour les coordonnées)
